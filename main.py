    ...
    if user_input == 'quit':
        ...
        continue
            ...
            if file:
                ...
                print(f'\nHouse: {response.text}\n')
                image_to_text = client.models.generate_content(
                    model="gemini-2.5-flash",
                    contents=[
                        'describe the content, also specifyng the file type',
                        myfile
                    ]
                )
                conversation_memory.append({"role": "user", "content": image_to_text.text}) # questa seconda elaborazione l'ho aggiunta perchè 
                conversation_memory.append({"role": "assistant", "content": response.text}) # altrimenti in memoria non vede cosa è stato caricato
    else:
